...
describe('SensorController - getLastPieceOfDataFromSensor', async function()  {
    it('sent nothing', function(done){
        testGetLastPieceOfDataFromSensor(undefined, 401, done);
    });

    it('sent smth, sensor not found', function(done){
        const stub1 = sinon.stub(Sensor, 'findOne')
        stub1.resolves(null)
        testGetLastPieceOfDataFromSensor("someSensorId", 400, () => { 
            done();
            stub1.restore();
        });
    });

    it('sent smth, sensor found', function(done){
       const stub1 = sinon.stub(Sensor, 'findOne')
        stub1.resolves({"externalIdentifier":"test"})
        const stub2 = sinon.stub(got,'')
        stub2.resolves({"resonse":"success"})
        testGetLastPieceOfDataFromSensor("someSensorId", 200, () => { 
            done();
           stub1.restore();
            stub2.restore();
        });
...
function testGetLastPieceOfDataFromSensor(sensorId, expectedStatusCode, done) {
    const req = {
        query: {
            sensorId: sensorId
        }
    ...
    SensorController.getLastPieceOfDataFromSensor(req, res, () => {}).then(function(result) {
        chai.expect(res.statusCode).to.equal(expectedStatusCode);
        done(result);
    })
}
